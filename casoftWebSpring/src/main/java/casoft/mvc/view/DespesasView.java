...
public class DespesasView {
    ...
    @PutMapping("/quitar")
    public ResponseEntity<Object> uptdeQuitar(@RequestParam int id,@RequestParam String valor, @RequestParam String data_venc, @RequestParam String data_lanc, @RequestParam String pagamento, @RequestParam String descricao, @RequestParam String status_conci, @RequestParam String tipoDespesa_id, @RequestParam String usuario_id, @RequestParam String evento_id,@RequestParam String data_pag,@RequestParam String obs,HttpServletRequest httpServletRequest){
        String token = httpServletRequest.getHeader("Authorization");
        if (token != null && token.startsWith("Bearer "))
            token = token.substring(7);
        if(JWTTokenProvider.verifyToken(token)) {
            Map<String,Object> json;
            json=controller.uptDespesaQuitar(id,valor,data_venc,data_lanc,pagamento,descricao,status_conci,tipoDespesa_id,usuario_id,evento_id,data_pag,obs);
            if (json!=null)
                return ResponseEntity.ok(json);
            else
                return ResponseEntity.badRequest().body(new Mensagem("Erro ao Cadastrar"));
    ...
    @PostMapping("/quitar")
    public ResponseEntity<Object> create(@RequestParam String valor, @RequestParam String data_venc, @RequestParam String data_lanc, @RequestParam String pagamento, @RequestParam String descricao, @RequestParam String status_conci, @RequestParam String tipoDespesa_id, @RequestParam String usuario_id, @RequestParam String evento_id,@RequestParam Integer pai_id,HttpServletRequest httpServletRequest){
        String token = httpServletRequest.getHeader("Authorization");
        if (token != null && token.startsWith("Bearer "))
            token = token.substring(7);
        if(JWTTokenProvider.verifyToken(token)) {
            Map<String,Object> json;
            json=controller.addDespesa(valor,data_venc,data_lanc,pagamento,descricao,status_conci,tipoDespesa_id,usuario_id,evento_id,pai_id);
            if (json!=null)
                return ResponseEntity.ok(json);
            else
                return ResponseEntity.badRequest().body(new Mensagem("Erro ao Cadastrar"));
